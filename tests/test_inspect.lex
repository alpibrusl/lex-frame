...
fn test_summary_nonempty() -> Int {
  if str.is_empty(inspect.summary(make_df())) {
    1
  } else {
    0
  }
...
fn test_to_markdown_nonempty() -> Int {
  if str.is_empty(inspect.to_markdown(make_df(), 10)) {
    1
  } else {
    0
  }
...
fn test_to_json_payload_nonempty() -> Int {
  if str.is_empty(inspect.to_json_payload(make_df(), 10)) {
    1
  } else {
    0
  }
...
fn run_all() -> Unit {
  let __lex_discard_1 := test_summary_nonempty() + test_summary_contains_shape() + test_to_markdown_nonempty() + test_to_markdown_has_header_sep() + test_to_json_payload_nonempty() + test_to_json_payload_has_schema() + test_null_report_nrows() + test_null_report_ncols() + test_history_nonempty_after_op() + test_sample_rows_nrows() + test_sample_rows_capped_at_nrows() + test_column_profile_nonempty()
  ()
}
