...
fn test_empty_nrows() -> Int {
  if frame.empty().nrows == 0 { 0 } else { 1 }
}

fn test_from_columns_nrows() -> Int {
  if make_df().nrows == 3 { 0 } else { 1 }
}

fn test_from_columns_ncols() -> Int {
  if list.len(make_df().col_names) == 2 { 0 } else { 1 }
}

fn test_head_nrows() -> Int {
  if frame.head(make_df(), 2).nrows == 2 { 0 } else { 1 }
}

fn test_head_all() -> Int {
  if frame.head(make_df(), 10).nrows == 3 { 0 } else { 1 }
}

fn test_tail_nrows() -> Int {
  if frame.tail(make_df(), 2).nrows == 2 { 0 } else { 1 }
}

fn test_slice_rows() -> Int {
  if frame.slice_rows(make_df(), 1, 3).nrows == 2 { 0 } else { 1 }
}

fn test_slice_empty() -> Int {
  if frame.slice_rows(make_df(), 2, 2).nrows == 0 { 0 } else { 1 }
}
