...
fn test_parse_csv_ncols() -> Int {
  match io.parse_csv("name,age\nAlice,25\nBob,30") {
    Ok(df) => if list.len(df.col_names) == 2 { 0 } else { 1 },
    Err(_) => 1,
  }
}
...
fn run_all() -> () {
  let _ := test_parse_csv_nrows() + test_parse_csv_ncols() + test_parse_csv_type_int() +
            test_parse_csv_empty_body_is_error() + test_render_csv_nonempty() +
            test_render_csv_roundtrip_nrows() + test_render_csv_roundtrip_ncols() +
            test_render_json_rows_nonempty() + test_render_json_rows_starts_with_bracket()
  ()
}
