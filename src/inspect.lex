...
fn to_json_payload(df :: frame.DataFrame, max_rows :: Int) -> Str {
  ...
  let schema_s := str.concat("{\"schema\": {\"columns\": {",
    str.concat(str.join(schema_entries, ", "),
      str.concat("}, \"shape\": [",
        str.concat(int.to_str(df.nrows),
          str.concat(", ", str.concat(int.to_str(list.len(df.col_names)), "]}"))))))
  ...
  match map.get(df.columns, column) {
    ...
    Some(c) => {
      ...
      let numeric_s := if is_numeric {
        let mean_s := match agg.mean_col(tmp, column) { Some(x) => float.to_str(x), None => "n/a" }
        let std_s  := match agg.std_col(tmp, column)  { Some(x) => float.to_str(x), None => "n/a" }
        let min_s  := match agg.min_col(tmp, column)  { Some(v) => val.to_str(v),   None => "n/a" }
        let max_s  := match agg.max_col(tmp, column)  { Some(v) => val.to_str(v),   None => "n/a" }
        str.join([
      ...
      if str.is_empty(numeric_s) { base }
      else { str.concat(base, str.concat("\n", numeric_s)) }
    },
  }
}
