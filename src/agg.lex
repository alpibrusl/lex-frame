# Column-level aggregation functions. All skip VNull unless stated otherwise.
...
fn numeric_floats(col :: List[val.Value]) -> List[Float] {
  list.fold(non_null(col), [], fn (acc :: List[Float], v :: val.Value) -> List[Float] {
    match val.as_float(v) { Some(f) => list.cons(f, acc), None => acc }
  })
}
...
fn count_all(df :: frame.DataFrame, _col :: Str) -> Int { df.nrows }
...
fn sum_col(df :: frame.DataFrame, col_name :: Str) -> Option[val.Value] {
  ...
  let nn := non_null(col)
  if list.is_empty(nn) { None }
  else if all_ints(col) {
    let total := list.fold(nn, 0, fn (acc :: Int, v :: val.Value) -> Int {
      match v { val.VInt(n) => acc + n, _ => acc }
    })
    Some(val.VInt(total))
  } else {
    let total := list.fold(nn, 0.0, fn (acc :: Float, v :: val.Value) -> Float {
      match val.as_float(v) { Some(f) => acc + f, None => acc }
    })
...
fn n_distinct(df :: frame.DataFrame, col_name :: Str) -> Int {
  let seen := list.fold(get_col(df, col_name), map.new(),
    fn (acc :: Map[Str, Bool], v :: val.Value) -> Map[Str, Bool] {
...
fn dedup_vals(col :: List[val.Value]) -> List[val.Value] {
  let res := list.fold(col, (map.new(), []),
    fn (acc :: (Map[Str, Bool], List[val.Value]), v :: val.Value) -> (Map[Str, Bool], List[val.Value]) {
      match acc {
        (seen, out) => {
  ...
  match res { (_, out) => list.reverse(out) }
}
