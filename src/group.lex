...
fn group_by(df :: frame.DataFrame, col :: Str) -> Result[GroupedFrame, frame.FrameError] {
  match map.get(df.columns, col) {
    ...
    Some(key_col) => {
      # Build Map[Str, List[Int]]: key_str → row indices.
...
fn agg(gf :: GroupedFrame, specs :: List[AggSpec]) -> frame.DataFrame {
  ...
  let key_col  := list.map(gf.groups, fn (g :: GroupEntry) -> Value { g.key_val })
  # Build one column per spec by applying the agg op to each group’s sub-column.
  let agg_cols := list.map(specs, fn (spec :: AggSpec) -> (Str, List[Value]) {
    let values := list.map(gf.groups, fn (g :: GroupEntry) -> Value {
      apply_agg_op(g.subframe, spec.in_col, spec.op)
    })
    (spec.out_col, values)
  })
  let all_cols := list.concat([(gf.group_col, key_col)], agg_cols)
  match frame.from_columns(all_cols) {
    Ok(df) => frame.record_op(df, prov.op_group_by(gf.group_col, n)),
    Err(_) => frame.empty(),
  }
}

# Apply a single AggOp to a column of a sub-frame.
fn apply_agg_op(sub :: frame.DataFrame, col :: Str, op :: AggOp) -> Value {
  match map.get(sub.columns, col) {
    None     => val.VNull,
    Some(xs) =>
      match op {
        AggSum          => agg.sum_col(xs),
        AggMean         => match agg.mean_col(xs) { Some(x) => val.VFloat(x), None => val.VNull },
        AggMin          => match agg.min_col(xs)  { Some(v) => v,             None => val.VNull },
        AggMax          => match agg.max_col(xs)  { Some(v) => v,             None => val.VNull },
        AggCount        => val.VInt(agg.count_all(xs)),
        AggCountNonNull => val.VInt(agg.count_non_null(xs)),
        AggStd          => match agg.std_col(xs)      { Some(x) => val.VFloat(x), None => val.VNull },
        AggVar          => match agg.variance_col(xs)  { Some(x) => val.VFloat(x), None => val.VNull },
        AggNDistinct    => val.VInt(agg.n_distinct(xs)),
      },
  }
}
