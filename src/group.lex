...
fn agg_sum() -> AggOp            { AggSum }
fn agg_mean() -> AggOp           { AggMean }
fn agg_min() -> AggOp            { AggMin }
fn agg_max() -> AggOp            { AggMax }
fn agg_count() -> AggOp          { AggCount }
fn agg_count_non_null() -> AggOp { AggCountNonNull }
fn agg_std() -> AggOp            { AggStd }
fn agg_var() -> AggOp            { AggVar }
fn agg_n_distinct() -> AggOp     { AggNDistinct }
...
fn group_by(df :: frame.DataFrame, col_name :: Str) -> Result[GroupedFrame, frame.FrameError] {
  match map.get(df.columns, col_name) {
    ...
    Some(c) => {
      ...
      let idx_map  := list.fold(list.enumerate(key_vals), map.new(),
      ...
      let groups := list.reverse(list.fold(map.entries(idx_map), [],
...
fn agg(gf :: GroupedFrame, specs :: List[AggSpec]) -> frame.DataFrame {
  let key_vals := list.map(gf.groups, fn (g :: GroupEntry) -> val.Value { g.key_val })
...
fn apply_agg_op(sub :: frame.DataFrame, col_name :: Str, op :: AggOp) -> val.Value {
  match op {
    AggSum          => match agg.sum_col(sub, col_name)      { Some(v) => v, None => val.vnull() },
    AggMean         => match agg.mean_col(sub, col_name)     { Some(f) => val.vfloat(f), None => val.vnull() },
    AggMin          => match agg.min_col(sub, col_name)      { Some(v) => v, None => val.vnull() },
    AggMax          => match agg.max_col(sub, col_name)      { Some(v) => v, None => val.vnull() },
    AggCount        => val.vint(agg.count_all(sub, col_name)),
    AggCountNonNull => val.vint(agg.count_non_null(sub, col_name)),
    AggStd          => match agg.std_col(sub, col_name)      { Some(f) => val.vfloat(f), None => val.vnull() },
    AggVar          => match agg.variance_col(sub, col_name) { Some(f) => val.vfloat(f), None => val.vnull() },
