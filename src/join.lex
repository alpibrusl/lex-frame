...
fn inner_join(left :: frame.DataFrame, right :: frame.DataFrame, on :: Str) -> Result[frame.DataFrame, frame.FrameError] {
  match (map.get(left.columns, on), map.get(right.columns, on)) {
    ...
    (Some(_), Some(_)) => {
      ...
      let right_key_col := match map.get(right.columns, on) {
        Some(c) => c,
        None => [],
      }
      let filled_rev := list.fold(frame.range_list(0, left.nrows), empty_map, fn (acc :: Map[Str, List[val.Value]], li :: Int) -> Map[Str, List[val.Value]] {
        let lk := frame.nth_value(match map.get(left.columns, on) {
          Some(c) => c,
          None => [],
        }, li)
...
fn cross_join(left :: frame.DataFrame, right :: frame.DataFrame) -> Result[frame.DataFrame, frame.FrameError] {
  ...
  let right_names := list.map(right.col_names, fn (n :: Str) -> Str {
    if list.fold(left_names, false, fn (acc :: Bool, ln :: Str) -> Bool {
      acc or ln == n
    }) {
      str.concat(n, "_right")
    } else {
  ...
  let all_names_rev := list.fold(right_names, list.fold(left_names, [], fn (a :: List[Str], s :: Str) -> List[Str] {
    list.cons(s, a)
  }), fn (a :: List[Str], s :: Str) -> List[Str] {
    list.cons(s, a)
  })
  ...
  let filled_rev := list.fold(frame.range_list(0, left.nrows), empty_map, fn (acc :: Map[Str, List[val.Value]], li :: Int) -> Map[Str, List[val.Value]] {
    list.fold(frame.range_list(0, right.nrows), acc, fn (acc2 :: Map[Str, List[val.Value]], ri :: Int) -> Map[Str, List[val.Value]] {
      let left_vals := list.map(left.col_names, fn (n :: Str) -> val.Value {
        frame.nth_value(match map.get(left.columns, n) {
          Some(c) => c,
          None => [],
        }, li)
      })
      let right_vals := list.map(right.col_names, fn (n :: Str) -> val.Value {
        frame.nth_value(match map.get(right.columns, n) {
          Some(c) => c,
          None => [],
        }, ri)
      })
      let row_vals_rev := list.fold(right_vals, list.fold(left_vals, [], fn (a :: List[val.Value], v :: val.Value) -> List[val.Value] {
        list.cons(v, a)
      }), fn (a :: List[val.Value], v :: val.Value) -> List[val.Value] {
        list.cons(v, a)
      })
...
fn build_index(df :: frame.DataFrame, col :: Str) -> Map[Str, List[Int]] {
  match map.get(df.columns, col) {
    None => map.new(),
    Some(xs) => list.fold(list.enumerate(xs), map.new(), fn (m :: Map[Str, List[Int]], p :: (Int, val.Value)) -> Map[Str, List[Int]] {
      let i := match p {
        (a, _) => a,
      }
      let v := match p {
...
fn result_col_names(left :: frame.DataFrame, right :: frame.DataFrame, on :: Str) -> List[Str] {
  ...
  let rev := list.fold(right_dis, list.fold(left_rest, [on], fn (a :: List[Str], s :: Str) -> List[Str] {
    list.cons(s, a)
  }), fn (a :: List[Str], s :: Str) -> List[Str] {
    list.cons(s, a)
  })
...
fn push_row(m :: Map[Str, List[val.Value]], names :: List[Str], row_vals :: List[val.Value]) -> Map[Str, List[val.Value]] {
  list.fold(list.enumerate(names), m, fn (acc :: Map[Str, List[val.Value]], p :: (Int, Str)) -> Map[Str, List[val.Value]] {
    let i := match p {
      (a, _) => a,
    }
    let name := match p {
...
fn append_pair(acc :: Map[Str, List[val.Value]], names :: List[Str], left :: frame.DataFrame, right :: frame.DataFrame, on :: Str, li :: Int, ri :: Int) -> Map[Str, List[val.Value]] {
  ...
  let on_val := frame.nth_value(match map.get(left.columns, on) {
    Some(c) => c,
    None => [],
  }, li)
  ...
  let row_vals_rev := list.fold(right_vals, list.fold(left_vals, [on_val], fn (a :: List[val.Value], v :: val.Value) -> List[val.Value] {
    list.cons(v, a)
  }), fn (a :: List[val.Value], v :: val.Value) -> List[val.Value] {
    list.cons(v, a)
  })
...
fn append_null_right(acc :: Map[Str, List[val.Value]], names :: List[Str], left :: frame.DataFrame, right :: frame.DataFrame, on :: Str, li :: Int) -> Map[Str, List[val.Value]] {
  ...
  let null_vals := list.map(right_rest, fn (_n :: Str) -> val.Value {
    val.vnull()
  })
  let row_vals_rev := list.fold(null_vals, list.fold(left_vals, [on_val], fn (a :: List[val.Value], v :: val.Value) -> List[val.Value] {
    list.cons(v, a)
  }), fn (a :: List[val.Value], v :: val.Value) -> List[val.Value] {
    list.cons(v, a)
  })
...
fn finalize_cols(names :: List[Str], col_map :: Map[Str, List[val.Value]]) -> List[(Str, List[val.Value])] {
  list.map(names, fn (name :: Str) -> (Str, List[val.Value]) {
    let col_rev := match map.get(col_map, name) {
      Some(xs) => xs,
      None => [],
    }
...
fn finish(filled :: Map[Str, List[val.Value]], names :: List[Str], left :: frame.DataFrame, right :: frame.DataFrame, on :: Str, kind :: Str) -> Result[frame.DataFrame, frame.FrameError] {
