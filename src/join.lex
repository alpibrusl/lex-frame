...
# Internally builds result rows as List[Value]; converts to typed cols
# at the end via frame.from_columns (which calls col_from_values).
...
fn inner_join(left :: frame.DataFrame, right :: frame.DataFrame, on :: Str) -> Result[frame.DataFrame, frame.FrameError] {
  match (map.get(left.columns, on), map.get(right.columns, on)) {
    ...
    (Some(_), Some(_)) => {
      ...
      let filled      := list.fold(frame.range_list(0, left.nrows), empty_acc,
...
fn cross_join(left :: frame.DataFrame, right :: frame.DataFrame) -> Result[frame.DataFrame, frame.FrameError] {
  let right_names := list.map(right.col_names, fn (n :: Str) -> Str {
    if list.fold(left.col_names, false, fn (acc :: Bool, ln :: Str) -> Bool { acc or ln == n }) {
      str.concat(n, "_right")
    } else { n }
  })
  ...
  let filled := list.fold(frame.range_list(0, left.nrows), empty_acc,
    fn (acc :: Map[Str, List[val.Value]], li :: Int) -> Map[Str, List[val.Value]] {
      list.fold(frame.range_list(0, right.nrows), acc,
        fn (acc2 :: Map[Str, List[val.Value]], ri :: Int) -> Map[Str, List[val.Value]] {
          let left_vals  := list.map(left.col_names,  fn (n :: Str) -> val.Value { col_nth_df(left,  n, li) })
          let right_vals := list.map(right.col_names, fn (n :: Str) -> val.Value { col_nth_df(right, n, ri) })
          let row := list.concat(
            zip_names(left.col_names, left_vals),
            zip_names(right_names, right_vals))
          push_row(acc2, row)
        })
    })
...
fn zip_names(names :: List[Str], vals :: List[val.Value]) -> List[(Str, val.Value)] {
  list.reverse(list.fold(list.enumerate(names), [],
    fn (acc :: List[(Str, val.Value)], p :: (Int, Str)) -> List[(Str, val.Value)] {
      let i    := match p { (a, _) => a }
...
fn build_index(df :: frame.DataFrame, name :: Str) -> Map[Str, List[Int]] {
  match map.get(df.columns, name) {
    ...
    Some(c) => {
      ...
      list.fold(list.enumerate(vals), map.new(),
        fn (m :: Map[Str, List[Int]], p :: (Int, val.Value)) -> Map[Str, List[Int]] {
          let i   := match p { (a, _) => a }
          let v   := match p { (_, b) => b }
          let k   := val.to_str(v)
          let xs  := match map.get(m, k) { Some(js) => js, None => [] }
...
fn result_col_names(left :: frame.DataFrame, right :: frame.DataFrame, on :: Str) -> List[Str] {
  ...
  let right_dis  := list.map(right_rest, fn (n :: Str) -> Str {
    if list.fold(left_rest, false, fn (acc :: Bool, ln :: Str) -> Bool { acc or ln == n }) {
      str.concat(n, "_right")
    } else { n }
  })
...
fn push_row(m :: Map[Str, List[val.Value]], row :: List[(Str, val.Value)]) -> Map[Str, List[val.Value]] {
  list.fold(row, m,
    fn (acc :: Map[Str, List[val.Value]], pair :: (Str, val.Value)) -> Map[Str, List[val.Value]] {
      let name     := match pair { (k, _) => k }
      let v        := match pair { (_, x) => x }
...
fn append_pair(acc :: Map[Str, List[val.Value]], names :: List[Str], left :: frame.DataFrame, right :: frame.DataFrame, on :: Str, li :: Int, ri :: Int) -> Map[Str, List[val.Value]] {
  ...
  let left_vals  := list.map(left_rest,  fn (n :: Str) -> val.Value { col_nth_df(left,  n, li) })
  let right_vals := list.map(right_rest, fn (n :: Str) -> val.Value { col_nth_df(right, n, ri) })
  let row := list.concat(
    list.cons((on, on_val), zip_names(left_rest, left_vals)),
    zip_names(disambig_right(left_rest, right_rest), right_vals))
  push_row(acc, row)
}
...
fn append_null_right(acc :: Map[Str, List[val.Value]], names :: List[Str], left :: frame.DataFrame, right :: frame.DataFrame, on :: Str, li :: Int) -> Map[Str, List[val.Value]] {
  ...
  let on_val     := col_nth_df(left, on, li)
  let left_vals  := list.map(left_rest,  fn (n :: Str) -> val.Value { col_nth_df(left, n, li) })
  let null_pairs := list.map(disambig_right(left_rest, right_rest), fn (n :: Str) -> (Str, val.Value) { (n, val.vnull()) })
...
fn finalize_cols(names :: List[Str], acc :: Map[Str, List[val.Value]]) -> List[(Str, List[val.Value])] {
  list.map(names, fn (name :: Str) -> (Str, List[val.Value]) {
    let col_rev := match map.get(acc, name) { Some(xs) => xs, None => [] }
