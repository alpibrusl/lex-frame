...
type Col = IntCol(List[Int]) | FloatCol(List[Float]) | StrCol(List[Str]) | BoolCol(List[Bool]) | NullableInt(List[Option[Int]]) | NullableFloat(List[Option[Float]]) | NullableStr(List[Option[Str]]) | NullableBool(List[Option[Bool]])

fn col_int(xs :: List[Int]) -> Col {
  IntCol(xs)
}

fn col_float(xs :: List[Float]) -> Col {
  FloatCol(xs)
}

fn col_str(xs :: List[Str]) -> Col {
  StrCol(xs)
}

fn col_bool(xs :: List[Bool]) -> Col {
  BoolCol(xs)
}

fn col_nullable_int(xs :: List[Option[Int]]) -> Col {
  NullableInt(xs)
}

fn col_nullable_float(xs :: List[Option[Float]]) -> Col {
  NullableFloat(xs)
}

fn col_nullable_str(xs :: List[Option[Str]]) -> Col {
  NullableStr(xs)
}

fn col_nullable_bool(xs :: List[Option[Bool]]) -> Col {
  NullableBool(xs)
}
...
fn nth_int(xs :: List[Int], i :: Int) -> val.Value {
  let m := list.fold(list.enumerate(xs), map.new(), fn (acc :: Map[Str, Int], p :: (Int, Int)) -> Map[Str, Int] {
    let idx := match p {
      (a, _) => a,
    }
    let v := match p {
...
fn nth_float(xs :: List[Float], i :: Int) -> val.Value {
  let m := list.fold(list.enumerate(xs), map.new(), fn (acc :: Map[Str, Float], p :: (Int, Float)) -> Map[Str, Float] {
    let idx := match p {
      (a, _) => a,
    }
    let v := match p {
...
fn nth_str(xs :: List[Str], i :: Int) -> val.Value {
  let m := list.fold(list.enumerate(xs), map.new(), fn (acc :: Map[Str, Str], p :: (Int, Str)) -> Map[Str, Str] {
    let idx := match p {
      (a, _) => a,
    }
    let v := match p {
...
fn nth_bool(xs :: List[Bool], i :: Int) -> val.Value {
  let m := list.fold(list.enumerate(xs), map.new(), fn (acc :: Map[Str, Bool], p :: (Int, Bool)) -> Map[Str, Bool] {
    let idx := match p {
      (a, _) => a,
    }
    let v := match p {
...
fn nth_opt_int(xs :: List[Option[Int]], i :: Int) -> val.Value {
  let m := list.fold(list.enumerate(xs), map.new(), fn (acc :: Map[Str, Option[Int]], p :: (Int, Option[Int])) -> Map[Str, Option[Int]] {
    let idx := match p {
      (a, _) => a,
    }
    let v := match p {
...
fn nth_opt_float(xs :: List[Option[Float]], i :: Int) -> val.Value {
  let m := list.fold(list.enumerate(xs), map.new(), fn (acc :: Map[Str, Option[Float]], p :: (Int, Option[Float])) -> Map[Str, Option[Float]] {
    let idx := match p {
      (a, _) => a,
    }
    let v := match p {
...
fn nth_opt_str(xs :: List[Option[Str]], i :: Int) -> val.Value {
  let m := list.fold(list.enumerate(xs), map.new(), fn (acc :: Map[Str, Option[Str]], p :: (Int, Option[Str])) -> Map[Str, Option[Str]] {
    let idx := match p {
      (a, _) => a,
    }
    let v := match p {
...
fn nth_opt_bool(xs :: List[Option[Bool]], i :: Int) -> val.Value {
  let m := list.fold(list.enumerate(xs), map.new(), fn (acc :: Map[Str, Option[Bool]], p :: (Int, Option[Bool])) -> Map[Str, Option[Bool]] {
    let idx := match p {
      (a, _) => a,
    }
    let v := match p {
...
fn col_from_values(vs :: List[val.Value]) -> Col {
  ...
  let has_null := match info {
    (a, _, _, _, _, _) => a,
  }
  let has_int := match info {
    (_, b, _, _, _, _) => b,
  }
  let has_float := match info {
    (_, _, c, _, _, _) => c,
  }
  let has_bool := match info {
    (_, _, _, d, _, _) => d,
  }
  let has_str := match info {
    (_, _, _, _, e, _) => e,
  }
  let has_other := match info {
    (_, _, _, _, _, f) => f,
  }
    ...
    if has_int and not has_float and not has_str and not has_bool and not has_other {
      ...
      if (has_int or has_float) and not has_str and not has_bool and not has_other {
        ...
        if has_null {
          ...
          StrCol(list.map(vs, fn (v :: val.Value) -> Str {
            val.to_str(v)
          }))
        }
      }
...
fn scan_value_types(vs :: List[val.Value]) -> (Bool, Bool, Bool, Bool, Bool, Bool) {
  list.fold(vs, (false, false, false, false, false, false), fn (acc :: (Bool, Bool, Bool, Bool, Bool, Bool), v :: val.Value) -> (Bool, Bool, Bool, Bool, Bool, Bool) {
    let hn := match acc {
      (a, _, _, _, _, _) => a,
    }
    let hi := match acc {
...
fn col_to_values(c :: Col) -> List[val.Value] {
  match c {
    IntCol(xs) => list.map(xs, fn (n :: Int) -> val.Value {
      val.vint(n)
    }),
    FloatCol(xs) => list.map(xs, fn (f :: Float) -> val.Value {
      val.vfloat(f)
    }),
    StrCol(xs) => list.map(xs, fn (s :: Str) -> val.Value {
      val.vstr(s)
    }),
    BoolCol(xs) => list.map(xs, fn (b :: Bool) -> val.Value {
      val.vbool(b)
    }),
...
fn filter_with_mask_int(xs :: List[Int], mask :: List[Bool]) -> List[Int] {
  let pairs := list.fold(list.enumerate(xs), [], fn (acc :: List[Int], p :: (Int, Int)) -> List[Int] {
    let i := match p {
      (a, _) => a,
    }
    let v := match p {
...
fn filter_with_mask_float(xs :: List[Float], mask :: List[Bool]) -> List[Float] {
  let pairs := list.fold(list.enumerate(xs), [], fn (acc :: List[Float], p :: (Int, Float)) -> List[Float] {
    let i := match p {
      (a, _) => a,
    }
    let v := match p {
...
fn filter_with_mask_str(xs :: List[Str], mask :: List[Bool]) -> List[Str] {
  let pairs := list.fold(list.enumerate(xs), [], fn (acc :: List[Str], p :: (Int, Str)) -> List[Str] {
    let i := match p {
      (a, _) => a,
    }
    let v := match p {
...
fn filter_with_mask_bool(xs :: List[Bool], mask :: List[Bool]) -> List[Bool] {
  let pairs := list.fold(list.enumerate(xs), [], fn (acc :: List[Bool], p :: (Int, Bool)) -> List[Bool] {
    let i := match p {
      (a, _) => a,
    }
    let v := match p {
...
fn filter_with_mask_opt_int(xs :: List[Option[Int]], mask :: List[Bool]) -> List[Option[Int]] {
  let pairs := list.fold(list.enumerate(xs), [], fn (acc :: List[Option[Int]], p :: (Int, Option[Int])) -> List[Option[Int]] {
    let i := match p {
      (a, _) => a,
    }
    let v := match p {
...
fn filter_with_mask_opt_float(xs :: List[Option[Float]], mask :: List[Bool]) -> List[Option[Float]] {
  let pairs := list.fold(list.enumerate(xs), [], fn (acc :: List[Option[Float]], p :: (Int, Option[Float])) -> List[Option[Float]] {
    let i := match p {
      (a, _) => a,
    }
    let v := match p {
...
fn filter_with_mask_opt_str(xs :: List[Option[Str]], mask :: List[Bool]) -> List[Option[Str]] {
  let pairs := list.fold(list.enumerate(xs), [], fn (acc :: List[Option[Str]], p :: (Int, Option[Str])) -> List[Option[Str]] {
    let i := match p {
      (a, _) => a,
    }
    let v := match p {
...
fn filter_with_mask_opt_bool(xs :: List[Option[Bool]], mask :: List[Bool]) -> List[Option[Bool]] {
  let pairs := list.fold(list.enumerate(xs), [], fn (acc :: List[Option[Bool]], p :: (Int, Option[Bool])) -> List[Option[Bool]] {
    let i := match p {
      (a, _) => a,
    }
    let v := match p {
...
fn col_concat(a :: Col, b :: Col) -> Col {
  match a {
    IntCol(xs) => match b {
      ...
      NullableInt(ys) => NullableInt(list.concat(list.map(xs, fn (n :: Int) -> Option[Int] {
        Some(n)
      }), ys)),
    ...
    FloatCol(xs) => match b {
      ...
      NullableFloat(ys) => NullableFloat(list.concat(list.map(xs, fn (f :: Float) -> Option[Float] {
        Some(f)
      }), ys)),
      _ => StrCol(list.concat(col_to_str_list(a), col_to_str_list(b))),
    },
    StrCol(xs) => match b {
      ...
      NullableStr(ys) => NullableStr(list.concat(list.map(xs, fn (s :: Str) -> Option[Str] {
        Some(s)
      }), ys)),
      _ => StrCol(list.concat(col_to_str_list(a), col_to_str_list(b))),
    },
    BoolCol(xs) => match b {
      ...
      NullableBool(ys) => NullableBool(list.concat(list.map(xs, fn (bv :: Bool) -> Option[Bool] {
        Some(bv)
      }), ys)),
      _ => StrCol(list.concat(col_to_str_list(a), col_to_str_list(b))),
    },
    NullableInt(xs) => match b {
      NullableInt(ys) => NullableInt(list.concat(xs, ys)),
      IntCol(ys) => NullableInt(list.concat(xs, list.map(ys, fn (n :: Int) -> Option[Int] {
        Some(n)
      }))),
      _ => StrCol(list.concat(col_to_str_list(a), col_to_str_list(b))),
    },
    NullableFloat(xs) => match b {
      NullableFloat(ys) => NullableFloat(list.concat(xs, ys)),
      FloatCol(ys) => NullableFloat(list.concat(xs, list.map(ys, fn (f :: Float) -> Option[Float] {
        Some(f)
      }))),
      _ => StrCol(list.concat(col_to_str_list(a), col_to_str_list(b))),
    },
    NullableStr(xs) => match b {
      NullableStr(ys) => NullableStr(list.concat(xs, ys)),
      StrCol(ys) => NullableStr(list.concat(xs, list.map(ys, fn (s :: Str) -> Option[Str] {
        Some(s)
      }))),
      _ => StrCol(list.concat(col_to_str_list(a), col_to_str_list(b))),
    },
    NullableBool(xs) => match b {
      NullableBool(ys) => NullableBool(list.concat(xs, ys)),
      BoolCol(ys) => NullableBool(list.concat(xs, list.map(ys, fn (bv :: Bool) -> Option[Bool] {
        Some(bv)
      }))),
      _ => StrCol(list.concat(col_to_str_list(a), col_to_str_list(b))),
    },
...
fn col_to_str_list(c :: Col) -> List[Str] {
  list.map(col_to_values(c), fn (v :: val.Value) -> Str {
    val.to_str(v)
  })
}
  ...
  match c {
    IntCol(xs) => {
      if list.len(xs) == 0 {
        ...
        Some(val.vint(list.fold(xs, 0, fn (acc :: Int, x :: Int) -> Int {
          acc + x
        })))
      }
    },
    FloatCol(xs) => {
      if list.len(xs) == 0 {
        ...
        Some(val.vfloat(list.fold(xs, 0.0, fn (acc :: Float, x :: Float) -> Float {
          acc + x
        })))
      }
    },
    NullableInt(xs) => {
      let r := list.fold(xs, (0, 0), fn (acc :: (Int, Int), o :: Option[Int]) -> (Int, Int) {
        let s := match acc {
          (a, _) => a,
        }
        let n := match acc {
      ...
      if count == 0 {
        ...
        Some(val.vint(match r {
          (a, _) => a,
        }))
      }
    },
    NullableFloat(xs) => {
      let r := list.fold(xs, (0.0, 0), fn (acc :: (Float, Int), o :: Option[Float]) -> (Float, Int) {
        let s := match acc {
          (a, _) => a,
        }
        let n := match acc {
  ...
  match c {
    IntCol(xs) => {
      ...
      if n == 0 {
        ...
        let s := list.fold(xs, 0, fn (acc :: Int, x :: Int) -> Int {
          acc + x
        })
    ...
    FloatCol(xs) => {
      ...
      if n == 0 {
        ...
        let s := list.fold(xs, 0.0, fn (acc :: Float, x :: Float) -> Float {
          acc + x
        })
  ...
  match col_mean(c) {
    ...
    Some(m) => {
      ...
      if n < 2 {
        ...
        let ss := match c {
          IntCol(xs) => list.fold(xs, 0.0, fn (acc :: Float, x :: Int) -> Float {
            let d := int.to_float(x) - m
            acc + d * d
          }),
          FloatCol(xs) => list.fold(xs, 0.0, fn (acc :: Float, x :: Float) -> Float {
            let d := x - m
            acc + d * d
          }),
          NullableInt(xs) => list.fold(xs, 0.0, fn (acc :: Float, o :: Option[Int]) -> Float {
            match o {
              None => acc,
              Some(x) => acc + (int.to_float(x) - m) * (int.to_float(x) - m),
            }
          }),
          NullableFloat(xs) => list.fold(xs, 0.0, fn (acc :: Float, o :: Option[Float]) -> Float {
            match o {
              None => acc,
              Some(x) => acc + (x - m) * (x - m),
            }
...
fn col_n_distinct(c :: Col) -> Int {
  match c {
    ...
    BoolCol(xs) => list.len(map.entries(list.fold(xs, map.new(), fn (m :: Map[Str, Bool], x :: Bool) -> Map[Str, Bool] {
      map.set(m, if x {
        "true"
      } else {
        "false"
    ...
    NullableStr(xs) => list.len(map.entries(list.fold(xs, map.new(), fn (m :: Map[Str, Bool], o :: Option[Str]) -> Map[Str, Bool] {
      match o {
        None => map.set(m, "null", true),
        Some(x) => map.set(m, x, true),
      }
    }))),
    NullableBool(xs) => list.len(map.entries(list.fold(xs, map.new(), fn (m :: Map[Str, Bool], o :: Option[Bool]) -> Map[Str, Bool] {
      match o {
        None => map.set(m, "null", true),
        Some(x) => map.set(m, if x {
          "true"
        } else {
          "false"
        }, true),
